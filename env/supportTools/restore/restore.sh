...
INSTANCE_NAME=$(echo "$CURRENT_DIR_NAME" | sed "s/\//_/g" | sed "s/[^a-zA-Z0-9_]//g")
...
function initVariables()
{
    ...
    then
        ...
        DBNAME=$(echo ${DBPREFIX}_${INSTANCE_NAME} | tr '[:upper:]' '[:lower:]');
...
function createDb
{
    mysqladmin --force -u$DBUSER --password=$DBPASS drop $DBNAME 2>/dev/null
    echo -n "Start create new DB ${DBNAME} - "
    mysqladmin -u$DBUSER --password=$DBPASS create $DBNAME 2>/dev/null
    echo "OK"
}
...
function restoreDb()
{
    ...
    then
        pv ${FILENAME_DB_DUMP} | gunzip -c | gunzip -cf | sed -e 's/DEFINER[ ]*=[ ]*[^*]*\*/\*/' | mysql -u$DBUSER --password=$DBPASS --force $DBNAME
    else
        gunzip -c $FILENAME_DB_DUMP | gunzip -cf | sed -e 's/DEFINER[ ]*=[ ]*[^*]*\*/\*/' | mysql -u$DBUSER --password=$DBPASS --force $DBNAME
    fi
...
function mysqlQuery()
{
    SQLQUERY_RESULT=$(mysql -u${DBUSER} --password=${DBPASS} --execute="${SQLQUERY}" 2>/dev/null);
}
...
{
    if [ -f ${MAGENTOROOT}.htaccess ] && [ ! -f ${MAGENTOROOT}.htaccess.merchant ]; then
...
function getMediaOrigHtaccess()
{
    ...
    if [ -f "${MAGENTO_FOLDER_MEDIA}/.htaccess" ] && [ ! -f ${MAGENTO_FOLDER_MEDIA}/.htaccess.merchant ]
    then
...
{
    if [ -f ${LOCALXMLPATH} ] && [ ! -f ${LOCALXMLPATH}.merchant ]; then
...
{
    if [ -f ${MAGENTOROOT}app/etc/enterprise.xml ] && [ ! -f ${MAGENTOROOT}app/etc/enterprise.xml.merchant ]; then
...
{
    if [ -f ${MAGENTOROOT}index.php ] && [ ! -f ${MAGENTOROOT}index.php.merchant ];then
